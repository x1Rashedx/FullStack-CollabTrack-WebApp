...
const TaskModal: React.FC<TaskModalProps> = ({ task, onClose, onUpdateTask, onDeleteTask, projectMembers, currentUser, isTeamAdmin, onCreateComment, onUploadAttachment, onDeleteAttachment }) => {
                ...
                >
                    ...
                    <aside className="space-y-6">
                        ...
                        <div>
                            ...
                            <div className="flex items-center justify-between p-2 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-700">
                                {(["low", "medium", "high"] as const).map((level) => (
                                    <button
                                        key={level}
                                        disabled={!isTeamAdmin}
                                        onClick={() => handleUpdate("priority", level)}
                                        className={`px-3 py-1 rounded-md text-sm font-semibold border transition-colors ${editableTask.priority === level ? priorityColors[level] + " border-transparent" : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 border-gray-300 dark:border-gray-600"} ${!isTeamAdmin ? "opacity-50 cursor-not-allowed" : ""}`}>
                         ...
                         <div className="relative">
                            ...
                            <div className={` ${isTeamAdmin ? 'flex flex-wrap gap-2' : ''}`}>
